...
void AnnotatedCameraWidget::drawLaneLines(QPainter &painter, const UIState *s) {
  ...
  if (sm["controlsState"].getControlsState().getExperimentalMode()) {
    ...
    start_hue = 60; 
    // straight: 70, in turns: 180
    end_hue = fmin(70 + orientation_future * 45, 180);

    // FIXME: painter.drawPolygon can be slow if hue is not rounded
    end_hue = int(end_hue * 100 + 0.5) / 100;
    bg.setColorAt(0.0, QColor::fromHslF(start_hue / 360., 0.51, 0.94, 0.8));
    bg.setColorAt(0.5, QColor::fromHslF(end_hue / 360., 0.68, 1.0, 0.5));
    bg.setColorAt(1.0, QColor::fromHslF(end_hue / 360., 0.68, 1.0, 0.0));
  }
